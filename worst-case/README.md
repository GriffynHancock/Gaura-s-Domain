...
**It embeds copies of the module pages, so it goes stale every time a module page changes** —
regenerate it after any edit to `public/crypto/**`, not just after edits to the builder.
...
all 21 challenges, with zero flag values, answers or solution hints (verified by grepping
the file for `flag{`, `florg{`, `glaf{`, `glorf{` and friends — the only surviving hit is
the XOR crib challenge, whose puzzle text *is* "every flag starts with `flag{`"). Reach
for this specifically in the
